...
multi sub filter-json(JSON::Path::CountryGrammar $path, $json where * ~~ Hash|Array, Bool :$want-path = False, :@path?, Callable :$assign --> Array) is export {
...
multi sub filter-json(Str $path, $json where * ~~ Hash|Array, Bool :$want-path = False, :@path?, Callable :$assign --> Array) is export {
  filter-json(JSON::Path::CountryGrammar.parse($path), $json, :$want-path, :@path, :$assign);
}
